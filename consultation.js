...
(() => {
  ...
  const validateAll = () => {
    ...
    if (firstErrorField) {
      const errorEl = form.querySelector(`[data-field="${firstErrorField}"]`);
      if (errorEl) {
  ...
  const getFormData = () => {
    ...
    return {
      ...
      homeSize:
        (form.querySelector('input[name="homeSize"]:checked') || {}).value ||
        "",
      propertyType:
  ...
  const initFormSubmit = () => {
    ...
    form.addEventListener("submit", (e) => {
      ...
      // Redirect immediately — must stay synchronous with the click/submit
      // for mobile browsers to treat it as a trusted user gesture
      ...
      // Cosmetic only — doesn't affect navigation since it already happened
      setTimeout(() => {
      ...
      .forEach((input) => {
        input.addEventListener("change", function () {
          ...
          if (content && this.checked) {
            content.style.boxShadow = "0 0 24px rgba(184,147,91,0.18)";
            setTimeout(() => {
